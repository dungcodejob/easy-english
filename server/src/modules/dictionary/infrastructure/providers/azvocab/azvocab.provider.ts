...
export class AzVocabProvider implements LookupProvider, IImportProvider {
  ...
  private readonly buildId = '_JSpT6LfxA-74vnvYXw9U';
  ...
  /**
   * Lookup a word (returns Word domain + rawData for LookupService to persist)
   */
  async lookup(word: string): Promise<Result<LookupResult, Error>> {
    try {
      const { vocabData, definitions } = await this.fetchWord(word);

      if (!vocabData || definitions.length === 0) {
      ...
      this.logger.error(`Lookup failed for ${word}: ${error.message}`);
  ...
  private async fetchWord(keyword: string): Promise<{
    vocabData: VocabDetailDto | null;
    definitions: AzVocabDefinitionResponseDto[];
  }> {
    const searchResults = await this.search(keyword);

    if (!searchResults || searchResults.length === 0) {
      return { vocabData: null, definitions: [] };
    }

    const firstEntry = searchResults[0];
    const definitions = await this.getDefinitions(firstEntry.defs || []);
  ...
  private async getDefinitions(
    ...
    for (const def of defs.slice(0, 10)) {
      const defData = await this.getDefinition(def.id);
      if (defData) {
